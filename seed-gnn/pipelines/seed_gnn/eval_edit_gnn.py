...
def eval_edit_gnn(config, debug=False):
    set_seeds_all(SEED)
    ...
    editable_param_count = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total_param_count = sum(p.numel() for p in model.parameters())
    logger.info(
        f'editable/tunable parameters: {editable_param_count} / {total_param_count}'
    )
    ...
    raw_results = process_edit_results(
        bef_edit_results,
        raw_results,
        editable_param_count=editable_param_count,
