...
def parse_args():
    ...
    parser.add_argument('--num_layers', type=int, default=None, help='Override model num_layers from pipeline config.')
    parser.add_argument('--gat_optimized', action='store_true',
                        help='Enable memory-optimized GAT construction (constant hidden size across layers).')
    parser.add_argument('--neighbor_batch_size', type=int, default=0,
                        help='If >0, enable NeighborLoader-based mini-batch pretraining with this seed-node batch size.')
    parser.add_argument('--neighbor_num_neighbors', type=int, default=10,
                        help='Neighbors sampled per hop when NeighborLoader pretraining is enabled.')
    parser.add_argument('--debug', action='store_true',
                        help='Enable verbose debug logging for runtime diagnostics.')
    
    # Feature ablation arguments
    parser.add_argument('--use_feature_ablation', action='store_true', help='whether to use feature ablation.')
    parser.add_argument('--feature_variant', type=str, default='full_features', help='name of the feature variant.')
    parser.add_argument('--drop_features', type=str, nargs='*', default=[], help='list of features to drop.')
...
def register_args_and_configs(args):
    ...
    if args.num_layers is not None:
        config['pipeline_params'].setdefault('architecture', {})
        config['pipeline_params']['architecture']['num_layers'] = args.num_layers
    if args.gat_optimized:
        config['pipeline_params'].setdefault('architecture', {})
        config['pipeline_params']['architecture']['gat_optimized'] = True
    if args.neighbor_batch_size and args.neighbor_batch_size > 0:
        config['pipeline_params'].setdefault('architecture', {})
        config['pipeline_params']['architecture']['neighbor_batch_size'] = args.neighbor_batch_size
        config['pipeline_params']['architecture']['neighbor_num_neighbors'] = args.neighbor_num_neighbors
    ...
    # Include optional sections from eval config (e.g., corruption)
    if 'corruption' in eval_config:
        config['corruption'] = eval_config['corruption']
    ...
    config['management']['seed'] = args.seed # Set seed in config from args
    config['management']['debug'] = bool(args.debug)
