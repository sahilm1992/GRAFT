...
class GAT(BaseGNNModel):
    def __init__(self, in_channels: int, hidden_channels: int,
                 ...
                 saved_ckpt_path: str = '', **kwargs):
        super(GAT, self).__init__(in_channels, hidden_channels, out_channels,
                                  num_layers, dropout, batch_norm, residual, use_linear)
        gat_optimized = kwargs.get("gat_optimized", False)
        num_heads = heads
        for i in range(num_layers):
            if gat_optimized:
                in_dim = in_channels if i == 0 else hidden_channels
                out_dim = out_channels if i == num_layers - 1 else hidden_channels
                layer_heads = 1 if i == num_layers - 1 else num_heads
                conv = GATConv(in_dim, out_dim, heads=layer_heads, concat=False, dropout=dropout)
            else:
    ...
    def forward(self, x: Tensor, adj_t: SparseTensor, *args, **kwargs) -> Tensor:
        # Check if we should use memory-efficient forward pass for large graphs
        # tfinance has ~21M edges, which causes OOM in GAT with 8 heads (40GB allocation)
        
        num_edges = 0
        if isinstance(adj_t, SparseTensor):
            num_edges = adj_t.nnz()
        elif isinstance(adj_t, Tensor) and adj_t.dim() == 2:
            num_edges = adj_t.size(1)

        # Heuristic for memory-intensive GAT layers
        # 21M edges * 8 heads * 32 dim * 4 bytes ~= 20GB per intermediate tensor
        # Total intermediate memory can reach 40GB+
        
        for idx in range(self.num_layers - 1):
            conv = self.convs[idx]
            
            # Memory-efficient GAT execution for large graphs
            if num_edges > 10000000:
                torch.cuda.empty_cache()
                
            # Use a smaller number of heads for large graphs during finetuning if OOM persists
            # For now, we'll try to just clear cache and hope for the best.
            # If it still OOMs, we might need to use gradient checkpointing.
    ...
    def __init__(self, in_channels: int, hidden_channels: int,
        ...
        if load_pretrained_backbone:
            ...
            self.GAT = GAT(in_channels=in_channels, hidden_channels=hidden_channels, out_channels=out_channels, \
                            num_layers=num_layers, heads = heads, dropout=dropout, batch_norm=batch_norm, residual=residual, **kwargs)
