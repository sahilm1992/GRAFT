...
class GIN(BaseGNNModel):
    ...
    def forward(self, x: Tensor, adj_t: SparseTensor, *args, **kwargs) -> Tensor:
        ...
        x = self.lin2(x)
        # For node regression (single output), return raw prediction.
        # Keeping log_softmax only for multi-class classification preserves
        # current behavior for existing classification experiments.
        if x.size(-1) == 1:
            return x
