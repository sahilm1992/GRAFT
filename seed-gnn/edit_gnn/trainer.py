...
class BaseTrainer(object):
    ...
    def get_optimizer(model_config, model, layers = None):
        ...
        if layers is not None:
            if model_config['optim'] == 'adam':
                optimizer = torch.optim.Adam(layers, lr=model_config['lr'], foreach=foreach)
            elif model_config['optim'] == 'rmsprop':
                optimizer = torch.optim.RMSprop(layers, lr=model_config['lr'], foreach=foreach)
            else:
                raise NotImplementedError
        else:
            if model_config['optim'] == 'adam':
                optimizer = torch.optim.Adam(model.parameters(), lr=model_config['lr'], foreach=foreach)
            elif model_config['optim'] == 'rmsprop':
                optimizer = torch.optim.RMSprop(model.parameters(), lr=model_config['lr'], foreach=foreach)
            else:
                raise NotImplementedError
    ...
    def bef_edit_check(self,  model, idx, label, curr_edit_target):
        ...
        torch.cuda.synchronize()
        input = self.grab_input(self.whole_data)
        if model.__class__.__name__ in ['GCN_MLP', 'SAGE_MLP', 'GCN2_MLP', 'GAT_MLP', 'JKNET_MLP', 'GIN_MLP', 'Polynormer_MLP']:
